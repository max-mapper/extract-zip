...
- `onEntryError` - function - if present, will be called with `(err, zipfile)`, err is every entry error from the zip file forwarded from the `error` event from yauzl. `zipfile` is the `yauzl` instance
